...
`)}});var require_boxen=__commonJS({"../../node_modules/boxen/index.js"(exports,module2){"use strict";var stringWidth=require_string_width(),chalk30=require("chalk"),widestLine=require_widest_line(),cliBoxes=require_cli_boxes(),camelCase=require_camelcase(),ansiAlign=require_ansi_align(),wrapAnsi=require_wrap_ansi(),NL=`
...
`),logger7.log(import_chalk4.default.cyan("https://github.com/storybookjs/react-native")),logger7.log();else{let storybookCommand=projectType==="ANGULAR"?`ng run ${installResult.projectName}:storybook`:packageManager.getRunStorybookCommand();if(logger7.log((0,import_boxen.default)(import_ts_dedent11.default`
...
\u{1F50E} found a '${import_chalk25.default.cyan(f.id)}' migration:`);let message=f.prompt(result);logger22.info((0,import_boxen3.default)(message,{borderStyle:"round",padding:1,borderColor:"#F1618C",title:f.promptOnly?"Manual migration detected":"Automigration detected"}));let runAnswer;try{if(dryRun)runAnswer={fix:!1};else if(yes)runAnswer={fix:!0},f.promptOnly&&(fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id));else if(f.promptOnly){fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id),logger22.info();let{shouldContinue}=await(0,import_prompts8.default)({type:"toggle",name:"shouldContinue",message:"Select continue once you have made the required changes, or quit to exit the migration process",initial:!0,active:"continue",inactive:"quit"},{onCancel:()=>{throw new Error}});if(!shouldContinue){fixResults[f.id]="manual_skipped";break}}else runAnswer=await(0,import_prompts8.default)({type:"confirm",name:"fix",message:`Do you want to run the '${import_chalk25.default.cyan(f.id)}' migration on your project?`,initial:!0},{onCancel:()=>{throw new Error}})}catch{break}if(!f.promptOnly)if(runAnswer.fix)try{await f.run({result,packageManager,dryRun,mainConfigPath,skipInstall}),logger22.info(`\u2705 ran ${import_chalk25.default.cyan(f.id)} migration`),fixResults[f.id]="succeeded",fixSummary.succeeded.push(f.id)}catch(error){fixResults[f.id]="failed",fixSummary.failed[f.id]=error.message,logger22.info(`\u274C error when running ${import_chalk25.default.cyan(f.id)} migration`),logger22.info(error),logger22.info()}else fixResults[f.id]="skipped",fixSummary.skipped.push(f.id)}else fixResults[f.id]=fixResults[f.id]||"unnecessary"}return{fixResults,fixSummary}}var import_chalk26=__toESM(require("chalk")),import_ts_dedent34=__toESM(require("ts-dedent")),import_semver25=__toESM(require("semver"));var logger23=console,getNextGlob=glob2=>{let extGlobsRegex=new RegExp(/(.*\.)(stories\.@.*)(\|mdx|mdx\|)(.*)$/i);if(glob2.match(extGlobsRegex))return glob2.replace(extGlobsRegex,"$1@(mdx|$2$4)");let allStoriesExtensionsRegex=new RegExp(/(.*\.)(stories\.\*)$/i);return glob2.match(allStoriesExtensionsRegex)?glob2.replace(allStoriesExtensionsRegex,"$1@(mdx|$2)"):glob2.replaceAll(".stories.mdx",".mdx")},bareMdxStoriesGlob={id:"bare-mdx-stories-glob",async check({storybookVersion,mainConfig}){if(!import_semver25.default.gte(storybookVersion,"7.0.0"))return null;let existingStoriesEntries=mainConfig.stories;if(!existingStoriesEntries)throw new Error(import_ts_dedent34.default`
      ...
      To learn more about this change, see: ${import_chalk26.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
...
`);return import_ts_dedent34.default`
    ...
    That would result in the following 'stories' config:
      ${import_chalk26.default.cyan(prettyNextStoriesEntries)}

    To learn more about this change, see: ${import_chalk26.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,mainConfigPath,result:{nextStoriesEntries}}){logger23.info(import_ts_dedent34.default`✅ Setting 'stories' config:
